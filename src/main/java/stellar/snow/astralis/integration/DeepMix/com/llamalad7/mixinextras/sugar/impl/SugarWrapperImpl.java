...
extends InjectorWrapperImpl {
    ...
    protected SugarWrapperImpl(InjectionInfo wrapper, MixinTargetContext mixin, MethodNode method, AnnotationNode annotation) {
        ...
        this.originalAnnotation = (AnnotationNode)Annotations.getValue(annotation, "original");
        method.visibleAnnotations.add(this.originalAnnotation);
        this.sugarAnnotations = (List)Annotations.getValue(annotation, "sugars");
        this.generics = new ArrayList<Type>(GenericParamParser.getParameterGenerics(method.desc, (String)Annotations.getValue(annotation, "signature")));
    ...
    private MethodNode prepareHandler(MethodNode original) {
        ...
        MethodNodeEx newMethod = new MethodNodeEx(original.access, MethodNodeEx.getName(original), original.desc, original.signature, original.exceptions.toArray(new String[0]), mixin);
