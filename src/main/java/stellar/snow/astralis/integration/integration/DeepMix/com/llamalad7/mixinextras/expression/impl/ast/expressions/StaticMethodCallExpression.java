...
extends SimpleExpression {
    ...
    protected boolean matchesImpl(FlowValue node, ExpressionContext ctx) {
        return MethodCallType.STATIC.matches(node) && this.name.matches(ctx.pool, node) && this.inputsMatch(node, ctx, ctx.allowIncompleteListInputs, this.arguments.toArray(new Expression[0]));
    }
}
