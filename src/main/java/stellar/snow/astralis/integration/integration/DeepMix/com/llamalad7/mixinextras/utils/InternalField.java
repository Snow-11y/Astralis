...
interface InternalField<O, T> {
    ...
    public static <O, T> InternalField<O, T> of(final Class<?> clazz, final String name) {
        Field impl;
        try {
        ...
        return new InternalField<O, T>(){
            ...
            public T get(O owner) {
                try {
                    ...
                    throw new RuntimeException(String.format("Failed to get %s::%s on %s! Please report to LlamaLad7!", clazz, name, owner), e);
                }
            }
    ...
    public static <O, T> InternalField<O, T> of(String clazz, String name) {
        try {
            return InternalField.of(Class.forName(clazz), name);
        }
